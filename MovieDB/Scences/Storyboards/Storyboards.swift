//
//  Storyboards.swift
//  MovieDB
//
//  Created by Bùi Xuân Huy on 10/24/19.
//  Copyright © 2019 huy. All rights reserved.
//

struct Storyboards {
    static let mainTabBar = UIStoryboard(name: "MainTabBar", bundle: nil)
    static let movie = UIStoryboard(name: "Movie", bundle: nil)
    static let categories = UIStoryboard(name: "Categories", bundle: nil)
    static let favourite = UIStoryboard(name: "Favourite", bundle: nil)
    static let started = UIStoryboard(name: "Started", bundle: nil)
    static let movieDetail = UIStoryboard(name: "MovieDetail", bundle: nil)
    static let playYoutubeTrailer = UIStoryboard(name: "PlayYoutubeTrailer", bundle: nil)
    static let searchMovie = UIStoryboard(name: "SearchMovie", bundle: nil)
    static let moreMovie = UIStoryboard(name: "MoreMovie", bundle: nil)
    static let movieByCategory = UIStoryboard(name: "MovieByCategory", bundle: nil)
}
